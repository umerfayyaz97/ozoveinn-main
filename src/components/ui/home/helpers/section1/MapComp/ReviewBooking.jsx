...
const ReviewBooking = ({ setComponent }) => {
  const {
    ...
    additionalVehicleCost: state.additionalVehicleCost,
  }));
  ...
  // Calculate additional options total (excluding "Add More Vehicles" and "Luggage Trailer")
  const additionalOptionsTotal = additionalOptions.reduce((acc, option) => {
    if (option === "Hourly Bookings") {
      return acc + vehicleDetails.hourlyRate * hourlyBookingCount;
    } else if (option === "Add More Vehicles" || option === "Luggage Trailer") {
  ...
  const totalDistance =
    distanceStartToEnd + distanceStartToStop + distanceStopToEnd;

  // Calculate additional fare for distance beyond the base 5km
  const distanceFare = totalDistance > 5 ? (totalDistance - 5) * perKmFare : 0;
  ...
  const totalAmount = perVehicleTotal + additionalVehicleCost;

  // Ensure consistency with Zustand's total price
  useEffect(() => {
    if (totalAmount !== totalPrice) {
      console.warn(
        "Total amount calculated does not match total price from Zustand store."
      );
    }
  ...
  const pricePerPerson =
    splitPaymentDetails?.passengers && splitPaymentDetails.passengers > 0
      ? (totalAmount / splitPaymentDetails.passengers).toFixed(2)
      : null;
      ...
      <div className="w-full p-4 lg:overflow-auto lg:h-[460px] text-black bg-white shadow-lg rounded-lg border border-gray-300 lg:border-none">
        ...
        <div className="mb-4">
          ...
          <div className="grid mt-2 gap-y-2">
            <div className="flex justify-between">
              <p className="text-xs">Base Fare</p>
              <p className="text-xs">${baseFare.toFixed(2)}</p>
            </div>
            <div className="flex justify-between">
              <p className="text-xs">Distance Charge</p>
