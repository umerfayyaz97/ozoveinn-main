...
export async function POST(request) {
  try {
    ...
    // Create Payment Intent with delayed capture
    const paymentIntent = await stripe.paymentIntents.create({
      amount: Math.round(totalPrice * 100), // total amount in cents
      currency: "usd",
      customer: customer.id,
      description: `Booking #${orderNumber} for ${vehicleName}`,
      capture_method: "manual", // This allows delayed capture
      metadata: {
        orderNumber,
        date,
        time,
    ...
    // Log success of Payment Intent creation
    console.log(
