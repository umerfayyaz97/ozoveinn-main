...
const useStore = create(
  devtools((set) => ({
    ...
    additionalVehicleCount: 0,
    luggageTrailerExtraAmount: 0, // Track the extra amount added for luggage trailers
    additionalVehicleCost: 0, // Track the cost of additional vehicles
    splitPaymentDetails: { passengers: 0 },
    orderNumber: "",
    splitPayment: false,
    options: [
    ...
    calculateTotalPrice: () =>
      set((state) => {
        ...
        // Total vehicle cost (for one vehicle)
        const totalVehiclePrice = baseFare + distanceFare;

        // Calculate additional options cost (excluding "Add More Vehicles" and "Luggage Trailer")
        const additionalOptionsCost = state.additionalOptions.reduce(
          (acc, option) => {
            if (option === "Hourly Bookings") {
              return (
        ...
        if (state.additionalOptions.includes("Add More Vehicles")) {
          additionalVehicleCost =
            perVehicleTotal * state.additionalVehicleCount;
        }

        // Calculate total price
        let totalPrice = perVehicleTotal + additionalVehicleCost;
